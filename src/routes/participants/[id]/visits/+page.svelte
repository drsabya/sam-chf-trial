...
<div class="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-emerald-100 selection:text-emerald-900 pb-32">
    
    <div class="max-w-xl mx-auto px-6 pt-8 flex justify-end items-center">
    ...
    <div class="max-w-xl mx-auto px-6 mt-12">
        ...
        <div class="flex flex-col items-center justify-center mb-16 space-y-4">
            ...
            <a href={`/participants/${participant.id}`} class="group flex items-center gap-2 text-[10px] font-bold tracking-[0.2em] text-slate-400 hover:text-emerald-600 transition-colors uppercase">
        ...
        <main class="space-y-4">
            {#if visits.length === 0}
                <div class="bg-white py-12 text-center border border-dashed border-slate-300 rounded-2xl">
                ...
                {#each visits as visit}
                    ...
                    >
                        <div class="flex items-center gap-5">
                            ...
                            <div class={`h-20 w-20 shrink-0 rounded-xl flex flex-col items-center justify-center border transition-colors duration-200 ${isCompleted ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-100 border-slate-200'}`}>
                                <span class={`text-[9px] font-bold uppercase tracking-widest mb-1 ${isCompleted ? 'text-emerald-600' : 'text-slate-400'}`}>
                                    Visit
                                </span>
                                <span class={`text-3xl font-light leading-none ${isCompleted ? 'text-emerald-700' : 'text-slate-600'}`}>
                                    {visit.visit_number}
                                </span>
                            </div>
                                ...
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    {:else}
                                        <div class="flex items-center gap-1.5 text-amber-600">
                                            ...
                                            <span class="text-[10px] font-bold uppercase tracking-widest">Due</span>
                                        </div>
                                    {/if}
                                </div>

                                <h3 class={`text-lg font-semibold tracking-tight ${isCompleted ? 'text-slate-900' : 'text-amber-600'}`}>
                                    {dateStr}
                                </h3>
                            </div>

                            <div class="pr-5 text-slate-300 group-hover:text-emerald-600 transition-colors">
