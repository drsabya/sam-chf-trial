...
<div class="min-h-screen bg-white font-sans text-slate-900 selection:bg-emerald-100 selection:text-emerald-900 pb-32">
	<div class="max-w-xl mx-auto px-6 pt-8 flex justify-between items-center">
		<a
			href="/participants"
			class="text-[10px] font-bold tracking-[0.2em] text-slate-300 hover:text-emerald-600 transition-colors uppercase"
		>
			&larr; Participants
		</a>
		<a
			href={`/participants/${participant.id}/visits`}
			class="group flex items-center gap-1 text-[10px] font-bold tracking-[0.1em] text-emerald-600 hover:text-emerald-800 transition-colors uppercase"
		>
			<span>Visits</span>
		...
		<form method="POST" action="?/update" class="space-y-10">
			<section class="space-y-8">
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
					<div class="group relative">
						...
						<label
							for="first_name"
							class="absolute left-0 -top-3.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-slate-300 peer-placeholder-shown:top-2 peer-focus:-top-3.5 peer-focus:text-[10px] peer-focus:text-emerald-600"
						>
							First Name
						</label>
					</div>
				...
				<div class="pt-4">
					<label
						for="phone"
						class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1"
			...
			{#if showDetails}
				<section transition:slide={{ duration: 300, axis: 'y' }} class="space-y-10 pt-6">
						...
						<div class="grid grid-cols-2 gap-6">
							<div>
								...
								<input
									...
									name="age"
									type="number"
									class="w-full bg-slate-50 rounded-lg px-3 py-2 text-sm border-transparent focus:bg-white focus:border-emerald-500 focus:ring-0 transition-colors"
							...
							<div>
								...
								>
									...
									<option value="Male" selected={currentValue('sex') === 'Male'}
										>Male</option
									>
									<option value="Female" selected={currentValue('sex') === 'Female'}
										>Female</option
									>
									<option value="Other" selected={currentValue('sex') === 'Other'}
										>Other</option
									>
								</select>
							</div>
						...
						<div class="grid grid-cols-1 gap-4">
							<div class="grid grid-cols-2 gap-4">
								<input
									...
									class="bg-slate-50 rounded-lg px-3 py-2 text-sm outline-none focus:bg-white focus:ring-1 focus:ring-emerald-500 transition-colors"
								/>
								<input
									type="text"
									name="occupation"
					...
					<div class="space-y-4">
						<div class="flex items-center gap-2 text-slate-300 border-b border-slate-50 pb-2">
							<h3 class="text-[10px] uppercase tracking-widest font-bold">Participant Status</h3>
						</div>
						<div class="flex flex-col sm:flex-row gap-4">
							<label class="inline-flex items-center gap-2 text-sm text-slate-700">
								<input
									...
									name="ltfu"
									type="checkbox"
									class="h-4 w-4 rounded border-slate-300 focus:ring-emerald-500"
									checked={Boolean(currentValue('ltfu', false))}
								/>
							...
							<label class="inline-flex items-center gap-2 text-sm text-slate-700">
								<input
									...
									type="checkbox"
									class="h-4 w-4 rounded border-slate-300 focus:ring-emerald-500"
									checked={Boolean(currentValue('consent_withdrawn', false))}
								/>
			...
			{#if message}
				<div
					class={`text-center text-xs font-medium py-4 ${
