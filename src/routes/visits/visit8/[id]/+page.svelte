<!-- src/routes/visits/visit8/[id]/+page.svelte -->
<script lang="ts">
	...
	async function uploadEcho(files: File[]) {
		...
		try {
			...
			// AI extraction for LVEF (Visit 8 echo)
			...
			try {
				const res = await fetch('/apis/visits/visit8/echo', {
					method: 'POST',
					body: fd
				});
				...
				if (!out.ok) {
					console.warn('Visit 8 echo extraction failed:', out.error);
				} else {
					extractionDialog = {
						title: 'Visit 8 echocardiography extracted',
				...
				console.error('Visit 8 echo endpoint error:', err);
			}
	...
	/* --------------------------------------------
	   Visit 8 — Efficacy Upload & Extraction
	--------------------------------------------- */
	...
	async function uploadEfficacy(files: File[]) {
		...
		try {
			...
			if (files.length === 1 && files[0].type === 'application/pdf') {
				...
				const scrId = participant.screening_id ?? '';
				const vNum = 'V8';
				const labelText = [init, scrId, vNum, 'EFF'].filter(Boolean).join(' ');
				finalFile = await convertFilesToSinglePdf(files, labelText);
			}
			...
			try {
				const res = await fetch('/apis/visits/visit8/efficacy', {
					method: 'POST',
					body: fd
				});
				...
				if (!out.ok) {
					console.warn('Visit 8 efficacy extraction failed:', out.error);
				} else {
					extractionDialog = {
						title: 'Visit 8 efficacy markers extracted',
						fields: [
							{ label: 'NT-proBNP', value: out.updated?.nt_pro_bnp ?? null },
							{ label: 'Serum TSH', value: out.updated?.serum_tsh ?? null },
							{ label: 'Serum homocysteine', value: out.updated?.serum_homocysteine ?? null },
	...
	/* --------------------------------------------
	   Visit 8 — Safety Upload & Extraction
	--------------------------------------------- */
	...
	async function uploadSafety(files: File[]) {
		...
		try {
			...
			try {
				const res = await fetch('/apis/visits/visit8/safety', {
					method: 'POST',
					body: fd
				});
				...
				if (!out.ok) {
					console.warn('Visit 8 safety extraction failed:', out.error);
				} else {
					extractionDialog = {
						title: 'Visit 8 safety labs extracted',
						fields: [
				...
				console.error('Visit 8 safety endpoint error:', err);
			}
...
<div class="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
	...
	<div class="max-w-2xl mx-auto pt-10 px-6">
		...
		<div class="space-y-8">
			...
			<!-- Echo & ECG (Visit 8) -->
			<section in:fly={{ y: 20, delay: 130, duration: 600 }}>
				<div class="flex items-center gap-2 mb-3 px-2">
				...
				>
					<div class="flex items-center justify-between mb-3 relative z-10">
						<h4 class="text-sm font-semibold text-slate-700">
							Efficacy report (NT-proBNP, TSH, homocysteine, GSH, TNF-α, IL-6, SAME, SAH, 5mC)
						</h4>
					...
					{:else}
						...
						>
							...
							<span class="text-sm text-slate-500 font-medium group-hover/label:text-emerald-700"
								>Tap to select combined efficacy report</span
							>
							<input
								type="file"
								class="hidden"
			...
			<!-- Safety upload (Visit 8) -->
			<section in:fly={{ y: 20, delay: 230, duration: 600 }}>
				<div class="flex items-center gap-2 mb-3 px-2">
			...
			<!-- Visit 8 conclusion (sets visit_date) -->
			<section in:fly={{ y: 20, delay: 280, duration: 600 }}>
				{#if visit.visit_date}
					<div
						class="bg-emerald-50/80 rounded-2xl border border-emerald-200 p-4 flex items-center gap-3 mt-4"
					>
				...
				{:else}
					<form
						method="POST"
						action="?/conclude"
						class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-3 mt-4"
					>
						...
						<p class="text-xs text-slate-500">
							On clicking the button, Visit 8 will be marked as completed using today’s date.
						</p>
						...
						>
							...
							<span>Mark Visit 8 Completed</span>
						</button>
					</form>
				{/if}
			</section>
