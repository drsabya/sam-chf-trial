// src/routes/apis/visits/visit6/echo/+server.ts
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const prompt = `
You are reading an echocardiography report (2D Echo) for a heart failure clinical trial VISIT 6.
...
- Do not include any other keys or text.
		...
		return json(
			{
				ok: false,
				error: err?.message ?? 'Unexpected error in Visit 6 echo endpoint'
			},
			{ status: 500 }
		);
	}
