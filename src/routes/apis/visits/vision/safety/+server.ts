// src/routes/apis/visits/vision/safety/+server.ts
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const visitNumberRaw = formData.get('visitNumber');
		...
		// derive a user-friendly label for prompt / logs
		const visitNumber =
			typeof visitNumberRaw === 'string' && visitNumberRaw.trim() !== ''
				? visitNumberRaw.trim()
				: 'X';
		...
		const prompt = `
You are reading a laboratory report for a heart failure clinical trial ${visitLabel}.
...
`.trim();
		...
		console.error('Generic safety vision endpoint error:', err);
		return json(
			{
				ok: false,
