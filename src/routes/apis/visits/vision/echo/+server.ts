// src/routes/apis/visits/vision/echo/+server.ts
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const prompt = `
You are reading a transthoracic echocardiography (2D Echo) report for a patient enrolled in a heart failure clinical trial.
...
- If multiple LVEF values exist, choose the one that appears to be the FINAL, CONCLUSION, or most recent value.
...
- Do not include any other keys or text.
		...
		console.error('Generic echo vision endpoint error:', err);
		return json(
			{
				ok: false,
				error: err?.message ?? 'Unexpected error in generic echo vision endpoint'
			},
			{ status: 500 }
		);
	}
