// src/routes/apis/visits/visit8/safety/+server.ts
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const prompt = `
You are reading a laboratory report for a heart failure clinical trial VISIT 8.
...
- Do NOT include any extra keys or text. Only the JSON object with these keys.
		...
		return json(
			{
				ok: false,
				error: err?.message ?? 'Unexpected error in Visit 8 safety endpoint'
			},
			{ status: 500 }
		);
	}
