// src/routes/apis/visits/visit2/vision/efficacy/+server.ts
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		// Prompt specifically for Visit 2 markers:
		// GSH, TNF-α, IL-6, SAME, SAH, 5-methylcytosine
		const prompt = `
You are reading a laboratory report for a heart failure clinical trial VISIT 2.
...
- Do NOT include any extra keys or text. Only the JSON object with these six keys.
		`.trim();
		...
		return json(
			{
				ok: false,
				error: err?.message ?? 'Unexpected error in Visit 2 efficacy vision endpoint'
			},
			{ status: 500 }
		);
	}
