<!-- src/lib/components/visits/VisitEcho.svelte -->
<script lang="ts">
	import {
		FileText,
		CloudUpload,
	...
	let {
		...
		participant,
		field = 'echo',
		visionEndpoint = '/apis/visits/vision/echo',
		sectionTitle = 'Echocardiography',
		sectionSubtitle = 'Echocardiography report (LVEF)'
	}: Props = $props();
	...
	async function uploadToR2(file: File) {
		try {
			...
			// update visit locally so parent sees new echo_src via $bindable
			if (field === 'echo') {
				visit = { ...visit, echo_src: objectKey };
			}
		} catch (err: any) {
	...
	async function uploadAndAnalyze() {
		...
		try {
			...
			// 1) Vision extraction (generic echo endpoint)
			...
			try {
				...
				if (!out.ok) {
					...
					extractionDialog = {
						title: 'Echocardiography LVEF extracted',
...
<section>
		...
		{#if uploading}
			...
			>
				...
				<span class="text-xs text-slate-500 font-medium">
					Analyzing echo report &amp; uploading…
				</span>
			</div>
		{:else if echoPublicUrl && files.length === 0}
			<!-- already uploaded -->
			<div
	...
	{#if visit.echo_lvef != null}
		<div class="mt-3 bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
			<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Stored LVEF</p>
			...
			<p class="mt-1 text-[11px] text-slate-400">
				Last updated on {formatDatePretty(visit.echo_lvef_updated_at ?? visit.updated_at ?? null)}
			</p>
		</div>
	{/if}
