<!-- src/lib/components/visits/VisitSafetyLabs.svelte -->
<script lang="ts">
	import {
		FileText,
		CloudUpload,
	...
	// We use $bindable() for 'visit' so mutations update the parent
	let {
		visit = $bindable(),
		participant,
		field = 'safety',
	...
	async function uploadToR2(file: File) {
		try {
			...
			// update visit locally so parent sees new safety_src via $bindable
			if (field === 'safety') {
				visit = { ...visit, safety_src: objectKey };
			}
		} catch (err: any) {
	...
	async function uploadAndAnalyze() {
		...
		try {
			...
			// derive visit label/number from visit object for both PDF label and dialog title
			const visitNumber =
				typeof visit?.visit_number === 'number' || typeof visit?.visit_number === 'string'
					? String(visit.visit_number)
					: '';
			const visitLabel = visitNumber ? `Visit ${visitNumber}` : 'Visit';
			...
			if (files.length === 1 && files[0].type === 'application/pdf') {
				...
				const vNum = visitNumber ? `V${visitNumber}` : 'V';
			...
			// send visitNumber to generic endpoint so it can customise the prompt
			if (visitNumber) {
				fd.append('visitNumber', visitNumber);
			}
			...
			try {
				...
				if (!out.ok) {
					...
					extractionDialog = {
						title: `${visitLabel} safety labs extracted`,
						fields: [
...
<section>
	...
	>
		...
		{:else if safetyPublicUrl && files.length === 0}
			<div
